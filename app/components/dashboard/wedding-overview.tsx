          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
            {weddingDetails?.weddingLocation && (
              <div className="flex items-center space-x-3 p-4 bg-rose-50 rounded-lg">
                <MapPin className="h-8 w-8 text-rose-500 flex-shrink-0" />
                <div>
            ...
            {weddingDetails?.guestCount && (
              <div className="flex items-center space-x-3 p-4 bg-purple-50 rounded-lg">
                <Users className="h-8 w-8 text-purple-500 flex-shrink-0" />
                <div>
            ...
            {weddingDetails?.budget && (
              <div className="flex items-center space-x-3 p-4 bg-green-50 rounded-lg">
                <PoundSterling className="h-8 w-8 text-green-500 flex-shrink-0" />
                <div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {weddingDetails?.culturalTraditions?.map((tradition: string) => (
                        <Badge key={tradition} variant="secondary" className="bg-rose-100 text-rose-700">
                          {tradition}
                        </Badge>
                      ))}
                    </div>
                  ...
                  <div>
                    ...
                    <div className="flex flex-wrap gap-2">
                      {weddingDetails?.religiousTraditions?.map((tradition: string) => (
                        <Badge key={tradition} variant="secondary" className="bg-blue-100 text-blue-700">
                          {tradition}
                        </Badge>
                      ))}
                    </div>
